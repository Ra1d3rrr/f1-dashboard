...
def get_all_sessions(year, rnd):
    """Load all sessions for a race weekend"""
    sessions = {}
    session_types = {'R': 'Race', 'Q': 'Qualifying', 'S': 'Sprint', 'SQ': 'Sprint Qualifying'}

    for session_type, name in session_types.items():
        try:
            session = fastf1.get_session(year, rnd, session_type)
            # Quick check if session exists without loading data
            if hasattr(session, 'session_info') or session.name:
...
# Get session info (but don't load data yet)
all_sessions = get_all_sessions(year, round_number)
session = all_sessions.get('R')
qualifying_session = all_sessions.get('Q')
sprint_session = all_sessions.get('S')
sprint_qualifying_session = all_sessions.get('SQ')
...
if page == "Lap Times (Live)":
    ...
    with tab1:
        ...
        if session:
            # Load session data only when this tab is accessed
        ...
        with tab4:
            ...
            if sprint_qualifying_session:
                ...
                with st.spinner("Loading sprint qualifying data..."):
                    try:
                        sprint_qualifying_session.load(laps=True, telemetry=False, weather=False, messages=False)
                        sprint_quali_laps = sprint_qualifying_session.laps
                    except:
                        sprint_quali_laps = pd.DataFrame()
...
# -------- PAGE: RACE RESULTS --------
    ...
    if session:
        try:
            # Load session data optimized for results
