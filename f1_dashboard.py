...
# -------- PAGE: RACE CONTROL --------
    ...
    if session:
        ...
        if not race_control_messages.empty and 'Message' in race_control_messages.columns:
            for idx, row in race_control_messages.iterrows():
                ...
                if any(word in message for word in ['penalty', 'penalised', 'penalized', 'time penalty', 'grid penalty', 'reprimand']):
                    ...
                    lap_match = re.search(r'lap (\d+)', message)
                    if lap_match:
                    ...
                    if 'Time' in row and pd.notna(row['Time']):
                        try:
                            penalty_info['Time'] = pd.to_datetime(row['Time']).strftime('%H:%M:%S')
                        except:
                            penalty_info['Time'] = str(row['Time'])
        ...
        if penalties_data:
            ...
            display_penalties = penalties_df[['Time', 'Driver', 'Car_Number', 'Penalty_Type', 'Lap', 'Penalty_Details']].copy()
            display_penalties.columns = ['Time', 'Driver', 'Car #', 'Penalty Type', 'Lap', 'Details']
        ...
        if not race_control_messages.empty:
            ...
            if 'Message' in messages_df.columns:
                ...
                display_columns.extend(['Message'])
